...
// Version: 
// Note: 代码量60K 加上预置音频大概占了 3个block 
// flash 8M 一共 16个block 上拉变声器分配 block 8 起始的 2个block 下拉录音器分配 block 5 起始的 2个block
// 1个block 是 64KB, 10秒录音大约是 23KB, 1个block 能存 30秒音频，13个block 大概能存 390秒 约 6分钟 的音频
//==========================================================================
//**************************************************************************
...
// 开机后通过首次 ADC 按键确定 当前处于 上拉或者下拉的连接方式
...
// 上拉模式自动监听式变声器状态处理函数
void Auto_PrepareRecord(void);		// 监听器准备录音，等待声音触发
void Auto_AttackStartRecord(void);	// 监听器触发录音
void Auto_StartPlayRecorded(void);  // 监听器触发变声播放
void Auto_StopWorkMode(void);		// 监听器停止工作

void CMPADC_Stop(void);			    // 停止 CMPADC 功能
...
// 麦克风检测大声时自动触发录音、静音时自动触发播放
volatile unsigned Env_AttackCount; // 达到攻击门槛值时次数加一，此时如果持续保持高于门槛值，攻击次数时不会连续增加的
volatile unsigned Env_ReleaseCount; // 达到释放门槛值时次数才会加一
...
// 系统状态变量
unsigned SystemMode = SYS_ADC_IDLE;
...
volatile unsigned KeyConnMode = KEY_CONN_UNKNOWN;
// 上拉按键锁存
volatile unsigned HighKeyLock = 0;
volatile unsigned HighKeyReleaseCnt = 0;
// unsigned waitCnt = 0;
//***************************************************************************************
// Main Function Area
//***************************************************************************************
...
void App_KeyScanTick(void)
{
	...
	case SYS_ADC_IDLE:
		...
		Handle_IOA7_ADC_Key();
		break;
	case SYS_HIGH_DIGITAL_MODE:
		// 处理上拉数字按键事件
		Handle_Key();
		break;
	case SYS_HIGH_WAIT_RELEASE:
		// 处理上拉数字按键等待释放事件
...
void Handle_Key(void)
{
	Key = SP_GetCh();
	/*
       如果已经处理过一次按下，则必须等物理松手后才允许下一次。
       注意：锁定期间仍然读取 SP_GetCh()，目的是清掉滴声播放期间积累的伪按键事件。
    */
	if (HighKeyLock)
	{
		if (IsHighKeyPressedRaw())
		{
			HighKeyReleaseCnt = 0;
		}
		else
	...
	if ((Key & 0x0080) != 0)
	{
		...
		/*
			一旦确认本次按键有效，立刻锁住。
			后面即使 PlayDiSound() 期间又扫描出 0x0080，也会被锁定逻辑吃掉。
		*/
		HighKeyLock = 1;
		HighKeyReleaseCnt = 0;
...
void Auto_StateMachine(void)
{
	switch (AutoState)
	{
		...
		case AUTO_WAIT_ATTACK:
			if (Env_AttackCount != LastAttackCount)
			{
				LastAttackCount = Env_AttackCount;
				LastReleaseCount = Env_ReleaseCount;
		...
		case AUTO_RECORDING:
			...
			if ((Env_ReleaseCount != LastReleaseCount)||(g_tma64Ticks >= MAX_REC_TIME))
			{
				LastReleaseCount = Env_ReleaseCount;
		...
		case AUTO_PLAYING:
			...
			else if (SystemMode == SYS_HIGH_DIGITAL_MODE)
			{
				Handle_Key();
			}
			break;
...
void PlayDiSound(void)
{
    ...
    *P_INT_Ctrl &= ~C_IRQ3_ADC;
	...
	SACM_VC4_AD_FIRType(C_ADC_FIR_Type0);
    SACM_VC4_DA_FIRType(C_DAC_FIR_Type0);
	// SACM_VC4_Volume_Control(C_Volume_Control_Enable);
    SACM_VC4_Volume(15535);
...
void Update_ADC_LowKey_Action(unsigned adcKey)
{
    ...
    else if (adcKey == ADC_KEY_NONE)
    {
        ...
        if (LowKey_State == 1)
        {
            ...
            if (hold >= ADC_LOW_KEY_SHORT_TICKS)
            {
...
void Handle_IOA7_ADC_Key(void)
{
	// 一旦进入上拉模式，后面无论 ADC 读到什么，都不可能再触发下拉长按录音
	if (KeyConnMode == KEY_CONN_HIGH)
	{
	...
	// if (SystemMode != KEYDOWN_LOW_IDLE)
	// 	return;
    ...
    if ((Last_IOA7_ADC_Key == ADC_KEY_NONE) && (adcKey != ADC_KEY_NONE))
    {
        ...
        if (adcKey == ADC_KEY_LOW_PRESS)
        {
			if (KeyConnMode == KEY_CONN_UNKNOWN)
			{
				KeyConnMode = KEY_CONN_LOW;
			}
		...
		{
			KeyConnMode = KEY_CONN_HIGH;

			// 新增一个中间状态，避免上拉按键模式时用户长按跳到低音调模式
			SystemMode = SYS_HIGH_WAIT_RELEASE;
...
void Handle_HighWaitRelease(void)
{
    ...
    if (adcKey == ADC_KEY_NONE)
    {
        ...
        // 进入上拉数字按键模式
        SystemMode = SYS_HIGH_DIGITAL_MODE;
